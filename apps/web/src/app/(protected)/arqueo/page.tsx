...
export default function ArqueoPage() {
  ...
  const filteredOperations = useMemo(() => {
    ...
    return operations.filter((operation) => {
      ...
      if (!term) return true;
      const haystack = [operation.borrowerName, operation.borrowerDni ?? '', operation.borrowerPhone ?? '']
        .join(' ')
        .toLowerCase();
    ...
    <div className="flex flex-col gap-6">
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <SummaryMetric label="Total cobrado" value={`$${summary.totalCollected.toFixed(2)}`} />
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="relative inline-flex w-full max-w-md items-center">
              <Input
                ...
                className="pl-10"
              />
              <span className="pointer-events-none absolute left-4 text-body-light/40">🔍</span>
            </div>
          </div>

          <div className="space-y-4">
            {filteredOperations.map((op) => (
              <OperationRow key={`${op.loanId}-${op.sequence}`} operation={op} onDetails={() => setSelectedOp(op)} />
            ))}
      ...
      <OperationModal operation={selectedOp} onClose={() => setSelectedOp(null)} />
    </div>
  );
}
...
function OperationRow({ operation, onDetails }: { operation: Operation; onDetails: () => void }) {
  ...
  return (
    <div className="grid gap-4 rounded-xl border border-white/25 bg-white/65 px-5 py-4 shadow-subtle backdrop-blur-lg md:grid-cols-4 md:items-center">
      <div className="md:col-span-2 space-y-1">
        <p className="text-sm font-semibold text-body-light">{operation.borrowerName}</p>
        <p className="text-xs text-body-light/60">DNI: {operation.borrowerDni || '—'}</p>
      </div>
      <div className="flex items-center gap-3">
        <Badge variant={badgeVariant}>{operation.status}</Badge>
        <span className="text-body-light/70 text-sm">${operation.amount.toFixed(2)}</span>
      </div>
...
function OperationModal({ operation, onClose }: { operation: Operation | null; onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm">
      <Card className="w-full max-w-lg shadow-pop">
        ...
        <CardContent className="space-y-4 text-sm text-body-light/80">
          ...
          />
          {operation.paidAt && (
            <DetailRow label="Pagado el" value={new Date(operation.paidAt).toLocaleString('es-AR')} />
          )}
          {operation.method && <DetailRow label="Método" value={operation.method} />}
        </CardContent>
