...
import { Badge, Card, CardContent, CardHeader, CardTitle, Input, Label, ActionButton } from '@prestaya/ui';
...
function buildOperations(loans: Awaited<ReturnType<typeof getLoans>>, payments: Awaited<ReturnType<typeof getPayments>>) {
  const paymentByInstallment = new Map(payments.map((payment) => [payment.installmentId ?? '', payment]));
...
export default function HistorialPage() {
  const [summary, setSummary] = useState({ totalCollected: 0, pendingInstallments: 0, overdueInstallments: 0 });
  ...
  useEffect(() => {
    async function load() {
      const [summaryData, payments, loans] = await Promise.all([getCashSummary(), getPayments(), getLoans()]);
  ...
  const allGroups = useMemo<OperationGroup[]>(() => {
    ...
    return Array.from(byLoan.values())
      .map((group) => ({
        ...group,
        operations: [...group.operations].sort((a, b) => a.sequence - b.sequence)
      }))
      .sort((a, b) => a.borrowerName.localeCompare(b.borrowerName, 'es', { sensitivity: 'base' }));
  }, [operations]);
    ...
    return allGroups
      .map((group) => {
        const operationsByStatus =
          statusFilter === 'ALL' ? group.operations : group.operations.filter((operation) => operation.status === statusFilter);
    ...
    <div className="flex flex-col gap-6">
      <Card className="transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-[2px] hover:shadow-hover">
        ...
        <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <SummaryMetric label="Total cobrado" value={formatCurrency(summary.totalCollected)} />
          <SummaryMetric label="Cuotas pendientes" value={summary.pendingInstallments} />
          <SummaryMetric label="Cuotas en mora" value={summary.overdueInstallments} />
        </CardContent>
      </Card>
      ...
      <Card className="transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-[2px] hover:shadow-hover">
        <CardHeader className="space-y-2">
          ...
          <p className="text-sm text-body-light/60">Consultá todos los movimientos del sistema.</p>
        </CardHeader>
        <CardContent className="flex flex-col gap-6">
          ...
          <div className="space-y-4">
            {filteredGroups.map((group) => (
              <LoanGroupCard key={group.loanId} group={group} onDetails={() => setSelectedLoanId(group.loanId)} />
            ))}
            {filteredGroups.length === 0 && (
              <p className="text-sm text-body-light/70">No encontramos operaciones con ese filtro.</p>
            )}
          </div>
        </CardContent>
      </Card>

      <LoanDetailsModal group={selectedGroup} onClose={() => setSelectedLoanId(null)} />
    </div>
  );
}
...
function StatusFilter({ filter, onChange }: { filter: 'ALL' | OperationStatus; onChange: (value: 'ALL' | OperationStatus) => void }) {
  const activeIndex = Math.max(0, STATUS_FILTERS.findIndex((option) => option.value === filter));
        ...
        return (
          <button
            ...
            className={clsx(
              ...
              isActive ? 'text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.25)]' : 'text-body-light/65 hover:text-body-light'
            )}
...
function LoanGroupCard({ group, onDetails }: { group: OperationGroup; onDetails: () => void }) {
  const paidCount = group.operations.filter((operation) => operation.status === 'PAID').length;
  const overdueCount = group.operations.filter((operation) => operation.status === 'OVERDUE').length;
  const pendingCount = group.operations.length - paidCount - overdueCount;
  const previewOperations = group.operations.slice(0, Math.min(group.operations.length, 3));
  const statusChips: Array<{ label: string; variant: 'warning' | 'danger' | 'success' }> = [];
  if (pendingCount === 0 && overdueCount === 0) {
    statusChips.push({ label: 'Completado', variant: 'success' });
  } else {
    if (pendingCount > 0) {
      statusChips.push({ label: `Pendientes ${pendingCount}`, variant: 'warning' });
    }
    if (overdueCount > 0) {
    ...
    <div className="space-y-4 rounded-xl border border-white/25 bg-white/65 px-6 py-5 shadow-subtle backdrop-blur-lg transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-[2px] hover:border-white/40 hover:bg-white/80 hover:shadow-hover">
      <div className="flex flex-col gap-3 transition-all duration-300 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1.5">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-body-light/60">
            ...
            <span>Emitido: {formatDate(group.operations[0]?.issuedAt ?? group.issuedAt)}</span>
          </div>
        </div>
      ...
      <div className="space-y-2">
        {previewOperations.map((operation) => (
          <LoanOperationPreview key={`${operation.loanId}-${operation.sequence}`} operation={operation} />
        ))}
...
function LoanOperationPreview({ operation }: { operation: Operation }) {
  const badgeVariant = operation.status === 'PAID' ? 'success' : operation.status === 'OVERDUE' ? 'danger' : 'warning';
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-white/20 bg-white/75 px-4 py-3 text-sm text-body-light transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-[1px] hover:border-white/35 hover:bg-white/85 hover:shadow-subtle md:flex-row md:items-center md:justify-between md:gap-6">
      <div className="space-y-1 md:flex-1">
        <p className="font-medium text-body-light">Cuota {operation.sequence}</p>
        <p className="text-xs text-body-light/60">Vence {formatDate(operation.dueDate)}</p>
        {operation.paidAt && (
          <p className="text-xs text-body-light/60">Pagada el {formatDate(operation.paidAt)}</p>
        )}
      </div>
      <div className="flex items-center justify-end gap-3 md:min-w-[150px]">
        ...
        <span className="font-semibold text-body-light tabular-nums min-w-[90px] text-right">{formatCurrency(operation.amount)}</span>
      </div>
    </div>
  );
}

function LoanDetailsModal({ group, onClose }: { group: OperationGroup | null; onClose: () => void }) {
  ...
  const overdueCount = group.operations.filter((operation) => operation.status === 'OVERDUE').length;
  const pendingCount = group.operations.length - paidCount - overdueCount;
  const totalAmount = group.operations.reduce((sum, operation) => sum + operation.amount, 0);
  const paidAmount = group.operations
    .filter((operation) => operation.status === 'PAID')
    .reduce((sum, operation) => sum + operation.amount, 0);
  const frequencyLabel = capitalizeFrequency(getFrequencyLabel(group.frequency));
      ...
      <Card className="flex max-h-[80vh] w-full max-w-xl flex-col overflow-hidden shadow-pop transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]">
        ...
        <CardContent className="flex-1 space-y-6 overflow-y-auto pt-6 pr-1">
          <div className="grid gap-3 md:grid-cols-2">
            <ModalMetric label="Total del préstamo" value={formatCurrency(totalAmount)} />
            <ModalMetric label="Total cobrado" value={formatCurrency(paidAmount)} />
            <ModalMetric label="Cuotas pagas" value={`${paidCount} de ${group.operations.length}`} />
          ...
          <div className="space-y-3">
            <p className="text-xs font-medium uppercase tracking-[0.12em] text-body-light/60">Detalle de cuotas</p>
            <div className="space-y-2">
              {group.operations.map((operation) => (
                <LoanInstallmentRow key={`${operation.loanId}-${operation.sequence}`} operation={operation} />
              ))}
            </div>
          </div>
        </CardContent>
  ...
  return (
    <div className="grid gap-3 rounded-xl border border-white/25 bg-white/70 px-4 py-3 text-sm text-body-light transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-[1px] hover:border-white/40 hover:bg-white/80 hover:shadow-subtle md:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_auto] md:items-center">
      <div className="space-y-1">
        <p className="font-semibold text-body-light">Cuota {operation.sequence}</p>
        <p className="text-xs text-body-light/60">Vence {formatDate(operation.dueDate)}</p>
        {operation.paidAt ? (
      ...
      <div className="text-right font-semibold text-body-light tabular-nums min-w-[100px]">{formatCurrency(operation.amount)}</div>
    </div>
  );
}
